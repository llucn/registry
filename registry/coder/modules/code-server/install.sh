...
install_deb() {
  ...
  fetch "${DOWNLOAD_BASE_LINK}/v$VERSION/code-server_${VERSION}_$ARCH.deb" \
...
install_rpm() {
  ...
  fetch "${DOWNLOAD_BASE_LINK}/v$VERSION/code-server-$VERSION-$ARCH.rpm" \
...
install_standalone() {
  ...
  fetch "${DOWNLOAD_BASE_LINK}/v$VERSION/code-server-$VERSION-$OS-$ARCH.tar.gz" \
